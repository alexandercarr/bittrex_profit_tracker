...
def update_table_values(table_body_win, table_body_string):
    ...
    updated_table_body = ''
    table_body_string_linesplit = table_body_string.split('\n')

    current_category_ind = 0
    updated_lines = []
    for key,entry in profit_tracker.profit_data.items():
        ...
        if ('CURRENT_PRICE_BTC' in entry):
            ...
            current_price_btc_string = "{0:.5f}".format(current_price_btc_string)
        ...
        line += "|"
        current_line_ind += 1
        avg_price_string = "{0:.4f}".format(entry['AVG_PURCHASE_PRICE_BTC'])
        ...
        if 'PROFIT_LOSS_USD' in entry:
            profit_loss_string = "{0:.4f}".format(entry['PROFIT_LOSS_USD'])
    ...
    try:
        ...
        pass
    table_body_win.refresh()
#def get_updated_line_data(table_body_line, data):
